...
**The ordering that makes this safe is the one this document already establishes.** `Connection.OnShutdownAsync` awaits every `PreConnectionShutdownAsync` handler before the public `ConnectionShutdownAsync` handlers, and automatic recovery starts from a public handler. The pre-shutdown chain reaches `SessionBase` teardown, which awaits `ConsumerDispatcher.ShutdownAsync(reason)`, which awaits the dispatcher's worker. So every consumer's `HandleChannelShutdownAsync` has already run, and set the reason, before recovery can re-register anything and clear it.

**That ordering is an assertion, not a contract, because nothing can currently falsify it.** The obvious guard - a test that records the order of the consumer's callbacks across a recovery and requires the shutdown before the re-registration - was written and then discarded, because it passes under the mutation it exists to catch. Measured: with `Connection.OnShutdownAsync`'s pre-shutdown `InvokeAsync` made fire-and-forget, both #2006 tests and the ordering test stayed green over repeated runs, at the fixture's 2 s `NetworkRecoveryInterval` and again at 1 ms. Two reasons. The first is an ordering property and holds only per session: `ConsumerDispatcherBase.ShutdownAsync` is not `async`, so `DoShutdownConsumers` runs to completion - queueing every shutdown item via `TryWrite` - before `InternalShutdownAsync` returns the incomplete `_worker`. It does *not* follow that all sessions' items are queued before the pre-shutdown chain first yields: `Channel.OnSessionShutdownAsync` awaits `OnChannelShutdownAsync` (arbitrary application handlers, plus a `_confirmSemaphore.WaitAsync`) *before* `ConsumerDispatcher.ShutdownAsync`, and `AsyncEventingWrapper` awaits handlers one at a time, so with several sessions a yielding handler suspends the chain before later sessions queue anything. The second reason is the one that actually carries the conclusion, and it is about margin: the competing write comes from a re-registration on a brand-new channel and dispatcher, reached only after `Task.Delay(NetworkRecoveryInterval)`, and costing a reconnect, a handshake, a `channel.open` and a `basic.consume` - orders of magnitude more than draining one already-queued work item. Making that race observable would mean stalling a dispatcher worker artificially, which tests the harness rather than the client.

So the sequential awaits are the right guarantee and should not be merged, but do not mistake the green suite for evidence that they are load-bearing: if they are removed, the timescales above will keep #2006 passing until something changes them. A test that cannot fail is worse than no test, which is why one was not added here.

**The reset is nevertheless conditional, and that matters.** It is skipped when the work item's cancellation token is already cancelled, which is the dispatcher's shutdown token that `Quiesce()` cancels. An unconditional reset leaves `ShutdownReason` null with `IsRunning` true on a permanently dead channel, which reads as fully healthy. Keeping the stale reason is the fail-safe answer, and it is what the code did before #2006.

**Be careful about *why* that interleaving is reachable, because the obvious answer is backwards.** The dispatcher queue is what rules it out at a dispatch concurrency of one, not what allows it: the queue is FIFO with a single reader, so a consume-ok already sitting in it is processed *before* the shutdown items `ShutdownAsync` writes afterwards, and `Quiesce()` sets `IsQuiescing`, so no new consume-ok is enqueued after that point. Two things reach it anyway. Above a concurrency of one, separate workers take the consume-ok and the shutdown item concurrently and nothing orders them at all. And at any concurrency the dispatcher's `IsQuiescing` check and its write are not atomic, so a consume-ok can pass the check, be preempted, and land *behind* the shutdown items: `ConsumerDispatcherBase.ShutdownAsync` calls `DoShutdownConsumers` and only then `InternalShutdownAsync`, which is what completes the channel, so a late write in between still succeeds.

The guard narrows the window rather than closing it. `IsCancellationRequested` is read and `_shutdownReason` written as two separate steps, so a worker can pass the check while the channel is still healthy, be preempted, and resume after another worker has recorded the shutdown, nulling the reason it just wrote. #2006 adds a new *cause* for that state, not a new state: `main` never wrote null, but `main` reaches the same observable pair by a different route, when a first registration's `AddConsumer` lands after `DoShutdownConsumers` has taken its `_consumers.ToArray()` snapshot, so no shutdown item is produced for that consumer at all and the consume-ok then sets `IsRunning` true with the reason never having been set.

**That residue is reproducible, not theoretical.** Driving the shipped `AsyncDefaultBasicConsumer`'s `HandleBasicConsumeOkAsync` and `HandleChannelShutdownAsync` concurrently in the order the dispatcher can produce above a concurrency of one - cancel the token, then record the reason, against a consume-ok that has already read the token - three runs of 400,000 trials each cleared the reason despite a recorded shutdown 881, 1060 and 1272 times. In 21, 185 and 59 of those the `_isRunning` write also landed last, giving the full null-reason-with-`IsRunning`-true state. **Treat the counts as machine-dependent rather than as a rate**: an independent reproduction on a 12-core box, same protocol, got 3510, 6083 and 2977 for the primary count and roughly a third of those for the `_isRunning` subset. What is not machine-dependent is that it is nowhere near zero. It belongs to #2016, where the unsynchronised state on this class is tracked. (The probe calls the two public methods directly; it demonstrates the class's two-step, not that the dispatcher schedules them that way.)

Note that `IsRunning` is set outside the guard, so on the guarded path the two properties end up disagreeing **permanently**, and an earlier revision of this section got that backwards by calling it a transient window. It is not. On that path the shutdown has by definition already reached the consumer - that is why there is a reason worth preserving - so the `IsRunning` false written by `OnCancelAsync` happens *before* the consume-ok writes it true again, and nothing writes it a third time: `OnCancelAsync` is reached only from `HandleBasicCancelAsync`, `HandleBasicCancelOkAsync` and `HandleChannelShutdownAsync`, all driven by dispatcher work items, and `InternalShutdownAsync` has already called `_writer.TryComplete()`, so `ShutdownConsumer`'s `TryWrite` produces nothing further. Measured deterministically: shutdown then consume-ok on the cancelled token leaves `ShutdownReason` non-null with `IsRunning` true, and it stays there for the life of the consumer. The reason is the property to trust; nothing will later reconcile `IsRunning` with it.
...
Neither test covers the shutdown-before-re-registration ordering that the reset depends on, and that is deliberate rather than an oversight - see the #2006 section above for the measurements showing why such a test passes under its own mutation.
