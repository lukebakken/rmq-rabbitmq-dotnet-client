...
**That ordering is an assertion, not a contract, because nothing can currently falsify it.** The obvious guard - a test that records the order of the consumer's callbacks across a recovery and requires the shutdown before the re-registration - was written and then discarded, because it passes under the mutation it exists to catch. Measured: with `Connection.OnShutdownAsync`'s pre-shutdown `InvokeAsync` made fire-and-forget, both #2006 tests and the ordering test stayed green over repeated runs, at the fixture's 2 s `NetworkRecoveryInterval` and again at 1 ms. Two reasons, and both are about margin rather than about the awaits. `ConsumerDispatcherBase.ShutdownAsync` queues the shutdown work items *synchronously*, so they are enqueued before `InvokeAsync` returns its first incomplete task at all. And the competing write comes from a re-registration on a brand-new channel and dispatcher, which costs a reconnect, a handshake, a `channel.open` and a `basic.consume` - orders of magnitude more than draining one already-queued work item. Making that race observable would mean stalling a dispatcher worker artificially, which tests the harness rather than the client.
