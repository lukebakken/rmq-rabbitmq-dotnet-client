...
**A related hazard, fixed here as well.** The entity-recovery loops follow a "release `_recordedEntitiesSemaphore`, await application code, re-acquire in a `finally`" pattern, and every re-acquire passed the recovery token. If the token is already cancelled the re-acquire throws, because `SemaphoreSlim.WaitAsync` observes the token before it looks at the count, so the `finally` completes without holding the semaphore and the outer `finally` in `TryPerformAutomaticRecoveryAsync` releases one it does not hold, raising `SemaphoreFullException` on a `SemaphoreSlim(1, 1)` that replaces the real cancellation - turning an orderly shutdown into a 541 internal error. Which handler is needed to reach it differed per loop: the exchange and binding loops need a `TopologyRecoveryExceptionHandler` whose condition matches, the queue loop needs only a `QueueNameChangedAfterRecovery` handler, and the consumer loop needed nothing at all until its release was guarded on `_recoveringConsumerAsyncWrapper.IsEmpty`. That guard narrowed the consumer case but did not remove it, because `RecoveringConsumerAsync` is public and any subscriber puts the release back.

All seven `_recordedEntitiesSemaphore` re-acquire sites - across the exchange, queue, queue-name-changed, binding and consumer loops - now pass `CancellationToken.None`. Fixing one and leaving six would have read as a fix while leaving the same failure reachable from the other three entity types.

Three waits are deliberately still cancellable, and none of them is a *semaphore-accounting* hazard, because each holds nothing at the moment it waits. The initial `_recordedEntitiesSemaphore` acquisition in `TryPerformAutomaticRecoveryAsync` is one; the two `_channelsSemaphore` waits in `RecoverChannelsAndItsConsumersAsync` are the others. **Do not read that as meaning cancelling them is free.** All three sit inside the same `try` whose `catch (Exception e)` calls `AbortAsync(Constants.InternalError, "FailedAutoRecovery", ...)`, so an `OperationCanceledException` from any of them is reported to the application as a 541 rather than as the orderly close it is. Nor are they the only route there: the setup RPCs in `AutorecoveringChannel.AutomaticallyRecoverAsync` - `basic.qos`, `tx.select`, and the `_rpcSemaphore.WaitAsync(k.CancellationToken)` inside each - propagate the same exception to the same catch-all, because only the topology-entity loops swallow it via `HandleTopologyRecoveryException`. #2037 tracks the whole class; it is a decision about how cancellation should be reported, not a token swap.

**Why untokenised waiting is safe on those seven, and the one place it is not.** Apart from the initial acquisition just described, every acquirer of `_recordedEntitiesSemaphore` is one of the ten `WaitAsync` sites on that field in `AutorecoveringConnection.Recording.cs` - the file has twelve in total, the other two being on `_channelsSemaphore` - and each holds it only across in-memory mutation of `_recordedExchanges` / `_recordedQueues` / `_recordedBindings` / `_recordedConsumers`; where one of them awaits, it awaits another `Recording` method passed `recordedEntitiesSemaphoreHeld: true`, which does not re-acquire. None performs I/O or invokes application code while holding it.

The exception is a pre-existing bug rather than a property of this change, and it is the initial acquisition's critical section that contains it. `TryPerformAutomaticRecoveryAsync` calls `RecoverChannelsAndItsConsumersAsync` with `recordedEntitiesSemaphoreHeld: true`; that loops over the recovered channels calling `AutomaticallyRecoverAsync`, which reaches `Channel.RunRecoveryEventHandlers` (declared in `Impl/Channel.cs`, invoked from `AutorecoveringChannel` on `_innerChannel`) and invokes user channel-recovery handlers **while the semaphore is held**. A handler that declares an exchange calls `RecordExchangeAsync`, which waits on that same semaphore with no token at all, and never returns - recovery is then wedged for the connection's lifetime. Confirmed on `main`, and reproduced twice independently: a probe registering an `IRecoverable.RecoveryAsync` handler that declares an exchange, with recovery forced by `rabbitmqctl close_all_connections`, stayed blocked past 25 seconds.
...
`projects/Test/Unit/TestRecordedEntityRecovery.cs` holds two tests covering two different failures.

`EveryRecordedEntityRecoveryAcceptsACancellationToken_GH1997` pins the set of entity `RecoverAsync` methods and requires each to take a *required* `CancellationToken`, so adding an entity, or defaulting the token, is a deliberate act. Verified by mutation: making the token optional fails the test. It is a signature check, so on its own it cannot see a method that accepts the token and then ignores it - keeping the parameter and passing `CancellationToken.None` in the body is a one-word change, it is the shape a refactor really takes, and before the behavioural test below existed it was measured to leave the whole suite green. It no longer does: under that mutation the suite now fails 1 of 225, which is exactly the test below.

`RecordedConsumerActuallyPassesTheTokenToBasicConsume_GH1997` closes that gap by pinning the body. It drives `RecordedConsumer.RecoverAsync` against a `DispatchProxy` `IChannel` that captures the token `basic.consume` was handed, asserts `basic.consume` was reached at all so the test cannot pass vacuously, and then asserts the captured token is the one supplied. `net8.0` only, because `DispatchProxy` does not exist on `net472`; the behaviour is framework-independent. Note the `#if NET` guard only ever bites in the `build-win32` job - `Unit.csproj` adds `net472` solely under `IsOSPlatform('Windows')`, so a Linux run targets `net8.0` alone and never compiles the excluded branch. Do not assert the token with `Assert.Equal`: `CancellationToken` does not override `ToString()`, so the failure prints `System.Threading.CancellationToken` for both expected and actual and says nothing - measured, and the reason the test asserts on `Equals` with a message naming `CanBeCanceled` and whether the captured token is `CancellationToken.None`.

What still has no test is the cancellation *latency* the fix improves. `TestToxiproxy.TestConsumerRecoveryContinuationTimeoutIsRetried_GH1993` already builds the necessary state (a `RecoveringConsumerAsync` handler plus a `Rate=0` toxic so the consume goes out but the reply cannot return), so adding a concurrent close with a stopwatch assertion there is the natural way to reach it.
