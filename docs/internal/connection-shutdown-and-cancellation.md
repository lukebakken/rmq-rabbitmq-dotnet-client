...
All seven `_recordedEntitiesSemaphore` re-acquire sites - across the exchange, queue, queue-name-changed, binding and consumer loops - now pass `CancellationToken.None`. Fixing one and leaving six would have read as a fix while leaving the same failure reachable from four other entity types.

Two waits are deliberately still cancellable. The initial acquisition in `TryPerformAutomaticRecoveryAsync` holds nothing yet, so cancelling there is correct and cheap. The two `_channelsSemaphore` waits in `RecoverChannelsAndItsConsumersAsync` are a different problem, and are the remaining reason a cancelled recovery can still surface as a 541: the catch-all above them cannot tell cancellation from failure. That needs a decision about how cancellation should be reported rather than a token swap, and is tracked by #2037.

**Why untokenised waiting is safe on those seven, and the one place it is not.** Every other acquirer of `_recordedEntitiesSemaphore` is one of the ten `WaitAsync` sites in `AutorecoveringConnection.Recording.cs`, and each holds it only across in-memory mutation of `_recordedExchanges` / `_recordedQueues` / `_recordedBindings` / `_recordedConsumers`; where one of them awaits, it awaits another `Recording` method passed `recordedEntitiesSemaphoreHeld: true`, which does not re-acquire. None performs I/O or invokes application code while holding it. The exception is a pre-existing bug rather than a property of this change: `TryPerformAutomaticRecoveryAsync` calls `RecoverChannelsAndItsConsumersAsync` with `recordedEntitiesSemaphoreHeld: true`, which reaches `AutorecoveringChannel.RunRecoveryEventHandlers` and invokes user channel-recovery handlers **while the semaphore is held**. A handler that declares an exchange calls `RecordExchangeAsync`, which waits on that same semaphore with no token at all, and never returns - recovery is then wedged for the connection's lifetime. Confirmed on `main`; a probe stayed blocked for 25 seconds. It cannot deadlock the seven re-acquires above, because it runs strictly after them on the same recovery task rather than concurrently.

This paragraph previously said the hazard was "not fixed by #1997" and pointed the remaining paths at #1995. That was written before the seven-site fix landed on this branch and was wrong once it did.
