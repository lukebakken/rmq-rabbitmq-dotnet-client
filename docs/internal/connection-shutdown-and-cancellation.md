...
Two things to know if you extend these tests. As of #1988 `AutorecoveringChannel.DisposeAsync` disposes its inner channel, so disposing the `IChannel` that `CreateChannelAsync` returns does reach `Channel`'s dispose path; the test captures `InnerChannel` beforehand only to inspect its semaphores afterwards. And use `BindingFlags.DeclaredOnly` when enumerating: `_rpcSemaphore` is `protected`, so `RecoveryAwareChannel` otherwise reports it a second time.

## Issue #1988: who owns the publisher-confirmation rate limiter

`AutorecoveringChannel` wraps a `RecoveryAwareChannel` and is what `CreateChannelAsync` returns whenever automatic recovery is enabled, which is the default. It never released anything the inner channel owned, so the inner channel's consumer dispatcher - a worker loop plus a `CancellationTokenSource`, created fresh per channel - was abandoned both when the wrapper was disposed and once per recovery cycle for the channel each recovery replaced.

The reason it could not simply dispose the inner channel was `Channel.DisposeAsyncCoreAsync` disposing the publisher-confirmation `RateLimiter`. That limiter belongs to the `CreateChannelOptions` instance it came from, not to any one channel: a caller-supplied limiter is shared across every channel created from those options, and a recovering channel reuses the same options for every recovery, so the replacement publishes through the very limiter its predecessor would have disposed. That was the root defect, and disposing only the dispatcher was a way around it rather than a fix for it.

So the ownership is corrected instead: a `Channel` no longer disposes the limiter, on either the sync or the async dispose path. The lifetime is the caller's. The library-created default (the `ThrottlingRateLimiter` on `CreateChannelOptions`, used when the caller supplies no options) is left to the garbage collector: it wraps a `ConcurrencyLimiter`, which was verified to hold no timer and no unmanaged handle, so there is nothing to reclaim. This is the same reasoning as the semaphores above, and it applies with more force here because the limiter has concurrent waiters on the publish path.

With that fixed, the wrapper disposes the whole inner channel: in `DisposeAsync`, and during recovery for the channel being replaced. The newly created channel is released from a lifetime `finally` in `AutomaticallyRecoverAsync` on every path that does not install it as the live channel - a failure in the setup that follows the creation - the `TakeOver`, which is an in-memory handover rather than an RPC, or the `basic.qos` or `tx.select` RPCs or the disposed-during-recovery branch - and unlike reaching into the dispatcher this also closes the channel on the broker. Those disposals are best-effort and log rather than throw, because they run in `finally` blocks where a throw would mask the exception already unwinding or fail an otherwise-successful recovery, and channel disposal can throw when the wait for a server-originated `channel.close` times out.

**That `finally` does not make every failed recovery leak-free, and it cannot.** A failure inside `CreateNonRecoveringChannelAsync` itself is not covered: the dispatcher and its worker tasks are built by the `Channel` constructor *before* `CreateAndOpenAsync` awaits `channel.open`, and neither that RPC nor `confirm.select` disposes the half-built channel on the way out, so the exception propagates with `newChannel` still `null` and the `finally` has nothing to release. Recovering against a node that has just restarted therefore still abandons one dispatcher per attempt. The fix belongs in `OpenAsync` / `CreateAndOpenAsync` and is not attempted here; the comment on that `try` says the same thing, and this paragraph previously claimed the `finally` meant "a failed recovery does not leak it", which is the case it misses.

A related defect surfaced while doing this and is fixed here too: `ThrottlingRateLimiter` overrode `Dispose(bool)` but not `DisposeAsyncCore()`. `RateLimiter.DisposeAsync` runs `DisposeAsyncCore` and then `Dispose(disposing: false)`, and the override's body was gated on `disposing`, so `await limiter.DisposeAsync()` disposed nothing: the inner `ConcurrencyLimiter` leaked and a "disposed" limiter kept handing out leases, making a use-after-dispose undetectable.

The consumer dispatcher's shutdown token is now captured once in its constructor rather than read from `_shutdownCts` per work item. `CancellationTokenSource.Token` throws `ObjectDisposedException` once the source is disposed, even if it was cancelled first, while a token already copied out stays usable. Reading it per work item meant a dispatcher `Dispose()` racing an inbound `basic.deliver` threw from the frame-receive loop, which nothing on that path catches, tearing down the entire connection rather than the one channel. Making dispatcher disposal reachable on the default recovery path is what would have exposed that.

Note on scope: `Channel.DisposeAsync` / `Channel.Dispose(bool)` are reachable through the public API for channel 0, which `Connection.DisposeAsync` disposes directly; what was previously unreachable was disposal of a channel obtained from `CreateChannelAsync` on a recovering connection.

One case is deliberately left open. A `DisposeAsync` racing an in-flight recovery can still leave a recovery-installed channel open on the broker, because `DisposeAsync` and `AutomaticallyRecoverAsync` read and mutate `_disposed` and `_innerChannel` without a shared lock: the recovery can pass its `_disposed` check and install the new channel just after `DisposeAsync` aborted the old one. Closing that pre-existing race needs synchronization between dispose and recovery - the same coordination that made connection shutdown deadlock-prone in #1960 and #1921 - so it is tracked separately, as issue #2020, rather than folded into this fix.
